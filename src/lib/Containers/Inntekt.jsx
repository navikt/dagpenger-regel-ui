...
const sumInntekter = (inntekter) =>
  inntekter.reduce((acc, val) => Number(acc) + Number(val.beloep), 0);
...
function Inntekt({ readOnly, inntekter, virksomhet, columnId, monthIndex }) {
  ...
  return (
    <>
      ...
      <div className={`item inntekter inntekter--${rowId}--${columnId}`}>
        <div className="bareprint">{`${virksomhet.navn} (${virksomhet.identifikator})`}</div>
        <Ekspanderbartpanel
          tittel={formatertPengesum(
            sumInntekter(
              inntekter.filter(
                (inntekt) => inntekt.virksomhet.identifikator === rowId
              )
            )
          )}
        >
                  ...
                  inntekter.map(
                    (inntekt, index) =>
                      inntekt.virksomhet.identifikator ===
                        ...
                        <div
                          key={`${inntekt.verdikode}${index}`}
                          className="flex inntekt"
                        >
                          <div>
                            <SelectField
                              bredde="xl"
                            ...
                            <div className="flexend">
                              ...
                              <OkAvbrytModal
                                ...
                                text={`Er du sikker på at du vil slette ${
                                  inntekt.verdikode
                                } - ${formatertPengesum(inntekt.beloep)}`}
                                avbrytCallback={() =>
                                  setSlettInntektModal(false)
                                }
                                OkCallback={() => {
                                  arrayHelpers.remove(index);
                                  setSlettInntektModal(
                                    !isSlettInntektModalOpen
                                  );
                                }}
                              />
                            </div>
...
Inntekt.propTypes = {
  inntekter: PropTypes.arrayOf(
    PropTypes.shape({
      header: PropTypes.string,
      label: PropTypes.string,
      beloep: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    })
  ).isRequired,
