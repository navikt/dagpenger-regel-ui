...
const renderRadioGroupField = RenderField(
  ({ label, name, value, onChange, bredde, readOnly, feil, children }) => {
    const optionProps = {
      name,
    ...
    return (
      <SkjemaGruppe
        ...
        className={`input--${bredde} radioGroup`}
      >
        {label.props.label && (
          <span className={`radioGroupLabel ${readOnly}`}>{label}</span>
        )}
...
export function RadioGroupField(props) {
  return <Field component={renderRadioGroupField} {...props} />;
}
...
const radioOptionsOnly = (options, key) => {
  ...
  if (option) {
    ...
    if (type.displayName !== RadioOption.displayName) {
      return new Error(
        'RadioGroupField children should be of type "RadioOption"'
      );
    }
  }
  return undefined;
