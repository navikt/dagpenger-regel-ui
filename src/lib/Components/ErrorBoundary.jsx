...
class ErrorBoundary extends React.Component {
  ...
  render() {
    ...
    if (apiErrors.hasError) {
      ...
      switch (error.status) {
        case 404:
          if (error.title) {
            ...
            feilmelding =
              "En feil har oppstått i forbindelse med tjenestekallet til inntekt";
          }
          break;
        case 418:
      ...
      return (
        <div className="feilmelding">{`Feilmelding: ${error.status} ${feilmelding}`}</div>
      );
    }
    if (hasError) {
